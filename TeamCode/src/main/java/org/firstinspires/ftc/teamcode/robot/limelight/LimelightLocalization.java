...
public class LimelightLocalization extends LLParent {
    ...
    public static class Params {
        ...
        public double maxUpdateTurretVelTicksPerSec = Math.toRadians(2);
    ...
    private Pose3D getRawCameraPoseFromLimelight() {
        ...
        for(LLResultTypes.FiducialResult tag : visibleTagInfo)
            if((robot.getAlliance() == Alliance.RED && tag.getFiducialId() == params.blueGoalId)
            || (robot.getAlliance() == Alliance.BLUE && tag.getFiducialId() == params.redGoalId))
                return null;
